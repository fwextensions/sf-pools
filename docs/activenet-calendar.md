...
Both the calendar and the catalog are ActiveNet endpoints — `sfrecpark.org` itself has no
API of any kind here. It's a CivicPlus CMS site; its pool pages are just links out to
ActiveNet and PDFs pulled from `/ImageRepository/Document?documentID=…`. There's no
evidence it proxies or duplicates the ActiveNet data, and no sign of a separate, private
system feeding either the calendar or the catalog — it's one vendor, one backend, two
modules within it (a curated calendar vs. the full activity search).

## Checking the activity catalog directly

Queried `POST /rest/activities/list` directly (same CSRF/cookie handshake as the
calendar) with `activity_keyword: "swim"`, paginating through all 50 results, to see
whether the catalog actually covers what the calendar misses.

**What it adds over the calendar:** the registered classes and teams (Learn to Swim,
Coffman Catfish, Garfield Gators, Garfield Jr. Gators, King Barracuda, Junior Barracuda,
Parent and Tot), each with a season-length `date_range` (e.g. `2026-08-18` to
`2026-12-12`) and a recurring `days_of_week` / `time_range`, instead of the calendar's
fixed ~3-week window.

**What's still missing, same as the calendar:**

- **Sava: zero activities.** Not on the calendar, not in the catalog either. It isn't
  thinly covered — it isn't configured in ActiveNet at all, at any level.
- **North Beach: still one activity** (`Adult Swim - Swimming for Fitness`, Sat
  4:30–5:00 PM), against the ~39 sessions its PDF lists.
- **No rentals, no SFUSD/school-district blocks**, in the catalog any more than the
  calendar. Those aren't things the public registers for, so there's no reason ActiveNet
  would carry them in *any* module — this gap isn't a curation gap, it's outside
  ActiveNet's scope entirely.

So the catalog is a better source than the calendar (full season, includes classes and
teams) but it is still a strict subset of the PDFs, not a replacement for them. There is
no single ActiveNet endpoint — calendar or catalog — that has everything the PDFs have.
...
Not as a schedule source — the calendar covers 6 of our 10 pools, three weeks at a time,
and even the fuller catalog still leaves Sava empty, North Beach nearly empty, and every
rental/SFUSD slot uncovered. It is worth more as a cross-check and as an enrichment:
...
## Is querying the catalog worth doing alongside PDF parsing?

As a *replacement* for the PDF parse, no — it can't be, given Sava, North Beach, and the
rental/SFUSD gaps above are permanent, not a snapshot artifact. But as a **supplement**
run alongside the PDF parse, it's plausibly worth it, for a narrow set of things the PDFs
don't have and never will:

- **Prices**, drop-in and tuition — the PDFs are schedule-only.
- **Registration links** per activity, for anything the PDF currently sends readers to
  ActiveNet's site to find.
- **A cheap staleness check.** Diffing the catalog's `date_range` for a pool against the
  PDF's season would catch a stale scrape the way it caught Garfield here, without
  needing OCR or a human to notice.
- **A "book lessons/teams" surface.** The catalog's registered-class entries could back a
  "here's how to sign up" panel next to a pool's schedule, since our own data has no
  registration info at all.

The cost is real, though: it needs the CSRF/cookie handshake against an undocumented
internal API, one more pool (Sava) it can't help with at all, and per-pool ambiguity
matching a catalog entry to the right PDF row (the `id` isn't stable across the two, only
day/time/pool). Given the gains are additive metadata rather than coverage, this is a
"nice to have if someone wants prices or registration links," not something the schedule
