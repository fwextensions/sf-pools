...
export default function ClosureNotice({ closure, poolName }: Props) {
	...
	return (
		<div className="plex-sans border-l-[3px] border-[#c98a1e] bg-[#fdf7ec] px-3 py-2.5">
			<div className="plex-mono text-label font-semibold tracking-[.14em] text-[#a9761c]">
				CLOSED
			</div>
			<p className="mt-1 text-body font-medium text-[#0e2733]">
				{poolName ? `${poolName} is closed` : "Closed"}
				{period ? ` ${period}` : ""}
				{closure.reason ? ` for ${closure.reason}` : ""}
			</p>
			<p className="mt-0.5 text-body text-[#5a707c]">
				{closure.endDate
			...
			{closure.sourceUrl ? (
				<a
					...
					target="_blank"
					rel="noreferrer"
					className="mt-1.5 inline-block plex-mono text-caption font-medium text-[#5a707c] underline underline-offset-2"
				>
					READ THE NOTICE ↗
				</a>
			) : null}
